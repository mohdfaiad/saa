...
OutFile "..\target\dist\${PRODUCT_NAME}-${PRODUCT_VERSION}-windows-installer.exe"
...
Section "${PRODUCT_NAME}" SEC01
  SetOutPath "$INSTDIR"
  SetOverwrite try
  File /r /x src "..\target\dist\${PRODUCT_NAME}-${PRODUCT_VERSION}-windows-installer\*.*"
...
Section -AdditionalIcons
  ...
  CreateShortCut "$SMPROGRAMS\$ICONS_GROUP\${PRODUCT_NAME} site.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
...
Section Uninstall
  ...
  MessageBox MB_YESNO "Do you want to delete the ${PRODUCT_NAME} configuration files too? ($PROFILE\.ireport)" IDNO configDone
    RMDir /r /REBOOTOK "$PROFILE\.ireport"
  configDone:
