 ...
 *
 * @author Slava Pestov
 * @version $Id: CTokenMarker.java 1088 2007-04-01 04:38:29Z gtoffoli $
 */
...
loop:		for(int i = offset; i < length; i++)
				...
				case '"':
					...
					break;
                                case '$':
                                        ...
                                        doKeyword(line, i, c);
                                        if (length - i > 2 &&
                                            (array[i1] == 'P' || array[i1] == 'F' || array[i1] == 'V' || array[i1] == 'R') &&
                                            array[i + 2] == '{')
                                        {
                                                addToken(i - lastOffset, token);
                                                lastOffset = lastKeyword  = i;
	...
	private boolean doKeyword(Segment line, int i, char c)
	{
		...
		byte id = keywords.lookup(line,lastKeyword,len);
                ...
                if (id == Token.NULL && keywordLookup != null)
                {
                         id = keywordLookup.lookup(line, lastKeyword, len);
                }
