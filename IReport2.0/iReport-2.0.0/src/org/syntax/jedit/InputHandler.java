 ...
 * 
 * @author Slava Pestov
 * @version $Id: InputHandler.java 1088 2007-04-01 04:38:29Z gtoffoli $
...
public abstract class InputHandler extends KeyAdapter
{
	...
	public static class clip_cut implements ActionListener {
		...
		{
			JEditTextArea textArea = getTextArea(evt);
                        if (textArea.getDocument().getUM().canUndo())
                        {
                            textArea.getDocument().getUM().undo();
                        }
		}
	}
		...
		{
			JEditTextArea textArea = getTextArea(evt);
                        if (textArea.getDocument().getUM().canRedo() )
                        {
                            textArea.getDocument().getUM().redo();
                        }
		}
	}
}
